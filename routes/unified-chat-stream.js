...
async function handleUnifiedChatStream(req, res, deps) {
 ...
 try {
   ...
   if (deepResearchContext) {
     enrichedSystemPrompt += `\n\n【深度研究结果】以下是多源并行搜索的结果。请严格基于这些搜索结果撰写研究报告。禁止编造搜索结果中没有的数据、百分比、事件。每个关键论点必须标注来源编号（如[联网-1][知识库-2]），如果某方面搜索结果中没有信息，写“本次搜索未找到相关数据”。直接引用原文而非概括性描述：${deepResearchContext}`;
   }
 } catch (drErr) {
 ...
 if (matchedToolStep && taskPlanner) {
   ...
   streamer.updateTaskPlanDesc(matchedToolStep.id, 'done', summary);
     ...
     if (deepResearchContext) {
       const researchReportPrompt = `你是一位专业的医美行业研究分析师。请严格基于以下搜索结果，撰写一份专业的研究报告。

【核心原则——禁止编造】
- 你只能使用下方【搜索结果】中提供的信息。任何搜索结果中没有的数据、百分比、年份、人名、事件，一律不得写入报告。
- 如果某个方面搜索结果中没有信息，写“本次搜索未找到相关数据”，绝对不要用你的内部知识填充。

【输出要求】
1. 每个关键论点必须标注来源编号，如 [联网-1][知识库-2]，不带来源编号的论点视为无效
2. 直接引用搜索结果中的原文、数据、事件，而不是概括性描述
3. 报告结构：概述 → 核心发现（分主题，每个主题引用具体来源）→ 关键数据/表格 → 总结与建议
4. 用中文回答，使用 Markdown 格式，层次清晰
5. 末尾附上参考来源列表（列出所有引用的来源编号和标题）
