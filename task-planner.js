...
const PLAN_TRIGGERS = [
  // 多步骤动词组合
  /帮(我|我们).*(分析|调研|整理|生成|制作|撰写|起草|规划|设计|优化|找|查|搜)/,
  /请.*(分析|调研|整理|生成|制作|撰写|起草|规划|设计|优化|找|查|搜)/,
  // 产出物关键词
  ...
  /第一.*第二.*第三/,
  // 复杂分析任务
  /(竞品|竞争对手|市场|行业).*(分析|调研|对比|报告|格局|趋势)/,
  /(制定|规划|设计).*(方案|策略|计划|流程)/,
  // 品牌/产品/公司相关的深度问题
  /(怎么做|如何做|怎样做).*(推广|营销|运营|销售|品牌|市场)/,
  /(进入|打入|开拓|布局).*(中国|市场|渠道|区域)/,
  // 对比/评估类
  /(对比|比较|区别|差异|优劣|评估|评价).*(产品|品牌|方案|策略)/,
  // 想知道/想了解 + 复杂主题
  /(想知道|想了解|想看看|告诉我).*(怎么|如何|策略|方案|打法|模式|路径)/,
  // 动词 + 从...到... 模式
  /从.*(到|至).*的.*(打法|策略|路径|方案|过程)/,
  // 包含"包括"的多维度问题
  /包括.*(和|与|以及|还有)/,
];
...
// ===== 不触发规划的排除词（简单问答）=====
const PLAN_EXCLUDES = [
  /^(你好|你是|谢谢|好的|嗯|哦|ok|OK|666|收到|明白)/,
  /^(什么是|怎么回事|为什么会).{0,10}$/,  // 只排除非常短的简单问答
  /画一张|展示一下|显示一下/,
  /^.{0,8}$/,  // 8字以内的超短消息直接排除
];
...
function needsPlanning(message) {
  ...
  if (message.length > 60 && (message.match(/[，,；;]/g) || []).length >= 2) return true;
  // 消息超过 30 字且包含疑问词 + 实体词（大概率是复杂问题）
  if (message.length > 30 && /[？?]/.test(message)) return true;
  if (message.length > 30) return true;  // 30字以上的消息在专家模式下默认触发
  return false;
}
